...
const router = new Hono<Env>()
	...
	.post('/research', orchestratorAgent.validator(), async (c) => {
	...
	// Input: { query: string, depth?: number, breadth?: number, maxResults?: number }
	.post('/orchestrator', orchestratorAgent.validator(), async (c) => {
		const data = c.req.valid('json');
		const result = await orchestratorAgent.run(data);
		return c.json(result);
	})
	...
	// Input: { query: string, depth: number, breadth: number, maxResults: number }
	.post('/researcher', researcherAgent.validator(), async (c) => {
		const data = c.req.valid('json');
	...
	.post('/web-search', webSearchAgent.validator(), async (c) => {
		const data = c.req.valid('json');
	...
	.post('/author', authorAgent.validator(), async (c) => {
		const data = c.req.valid('json');
