...
const PHASE_LABELS: Record<string, string> = {
	...
	booting: 'Starting OpenCode server',
		...
		const poll = async () => {
			while (!cancelled) {
				...
				try {
					const res = await fetch('/api/status');
					if (!res.ok) continue;
					const status: { exists?: boolean; phase?: string; error?: string } = await res.json();
					if (cancelled) break;

					if (!status.exists) {
						setError('Workspace setup was interrupted.');
						setState('idle');
						break;
					}
					...
					if (status.phase === 'error') {
						setError(status.error || 'Workspace setup failed.');
						setState('idle');
						break;
					}
				} catch {
				...
				{state === 'booting' && (
					<div className="bg-black border border-gray-900 rounded-lg p-8 flex flex-col items-center gap-4">
						...
						<p className="text-white text-sm" data-loading="true">
							{phase ? PHASE_LABELS[phase] || 'Starting workspace' : 'Starting workspace'}
						</p>
					</div>
				)}
