...
import adkTime from '../agent/adk-time';
...
// Call the Google ADK bridge agent through Agentuity
api.post('/adk-time', adkTime.validator(), async (c) => {
        const data = c.req.valid('json');

        return c.json(await adkTime.run(data));
});
