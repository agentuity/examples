...
if (!process.env.OPENAI_API_KEY || process.env.OPENAI_API_KEY === process.env.AGENTUITY_SDK_KEY) {
  {
    ...
    if (url && apikey) {
      ...
      process.env.OPENAI_BASE_URL = url + "/gateway/openai";
      console.debug("Enabled Agentuity AI Gateway for openai");
    } else {
      if (true) {
        console.warn("\nYou have not set the environment variable OPENAI_API_KEY in your project .env file.\n");
      } else {
        console.warn('\nYou have not set the environment variable OPENAI_API_KEY in your project. Use "agentuity env set OPENAI_API_KEY" to set it and redeploy your project.\n');
      }
      process.exit(1);
    }
  }
...
// src/agents/rfpGenerator/index.ts
...
var exampleRfp = {
  ...
  Introduction: {
    type: "string",
    content: "We are seeking proposals for a complete redesign of our corporate website.",
    description: "Brief background and purpose of the RFP"
  },
  "Project Overview": {
    type: "string",
    content: "The project includes a new design, improved UX, and integration with our CMS.",
  ...
  "Submission Instructions": {
    type: "string",
    content: "Submit proposals via email to jane.doe@example.com by 5pm on July 15, 2024.",
...
var welcome = () => {
  return {
    welcome: "Welcome to the PDF Generator Agent! I can help you generate a professional RFP Markdown document from your JSON data.",
    prompts: [
      {
        data: exampleRfp,
...
async function Agent(req, resp, ctx) {
  try {
...
${JSON.stringify(rfpData, null, 2)}
    ...
    const markdown = completion.choices?.[0]?.message?.content || "Error generating RFP Markdown.";
