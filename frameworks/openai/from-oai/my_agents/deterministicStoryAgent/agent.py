...
async def run(request: AgentRequest, response: AgentResponse, context: AgentContext):  
    ...
    try:
        ...
        with trace("Deterministic story flow"):  
            ...
            outline_result = await Runner.run(story_outline_agent, input_prompt, context=context)  
            ...
            # 4. Write the story  
            story_result = await Runner.run(story_agent, outline_result.final_output, context=context)  
        ...
        return response.json({
            "status": "error",
            "reason": "Internal server error while generating story"
        })