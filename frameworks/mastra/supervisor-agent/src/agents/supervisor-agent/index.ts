...
import { Agent as MastraAgent, Mastra, createTool } from '@mastra/core';
import { z } from 'zod';
...
export const welcome = () => {
  return {
    welcome:
      'Welcome to the Mastra Supervisor Agent! This demonstrates a multi-agent system where a publisher agent supervises a copywriter and editor to create polished blog posts. The publisher coordinates the workflow: first calling the copywriter to draft content, then the editor to refine it.',
    prompts: [
      {
...
export default async function Agent(
  ...
  try {
    ...
    const topic = userInput || 'React JavaScript frameworks';

    ctx.logger.info(`Processing request for topic: ${topic}`);

    const agent = mastra.getAgent('publisherAgent');
    const response = await agent.generate(
      `Write a blog post about ${topic}. Only return the final edited copy.`
    );
