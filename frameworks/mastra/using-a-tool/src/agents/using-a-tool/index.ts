...
export const welcome = () => {
  return {
    welcome:
      'Welcome to the London Weather Agent! I can provide historical weather information for London from January 1st of this year up to today.',
    prompts: [
      {
        data: 'How many times has it rained in London this year?',
        contentType: 'text/plain',
      },
      {
        data: 'What was the coldest day in London so far this year?',
        contentType: 'text/plain',
      },
    ],
  };
...
- If the question cannot be answered with available data, say so clearly.`,
    ...
    const result = await londonWeatherAgent.generate(
      (await req.data.text()) ?? 'What is the weather in London?'
    );
